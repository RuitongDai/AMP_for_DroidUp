...
F2_MOTION_DIR = f"{LEGGED_GYM_ROOT_DIR}/humanoid/envs/datasets/x3_f2"
...
Terrain_point_x, Terrain_point_y = create_point_list(resolution=0.1,
                                                     ...
                                                     range_y=(-0.5, 0.5),
                                                     debug=True)
class X3F2AMPCfg(LeggedRobotCfg):
    """
    ...
    class control(LeggedRobotCfg.control):
        ...
        dof_vel_max = [
            6.28, 5.44, 5.44, 12.56, 17.80, 4.19,
            6.28, 5.44, 5.44, 12.56, 17.80, 4.19,
            5.44, 5.44,
        ]
...
Short_obs = 10 * X3F2AMPCfg.env.num_single_obs # input actor
Long_obs =  10 * X3F2AMPCfg.env.num_single_obs # input state_encoder

class X3F2AMPCfgPPO(LeggedRobotCfgPPO):
    seed = 1
    ...
    class algorithm(LeggedRobotCfgPPO.algorithm):
        ...
        class symmetry_cfg:
            ...
            frame_stack = X3F2AMPCfg.env.frame_stack
            sym_coef = 1.0

        # -- est cfg
        class priv_est_cfg:
            obs_recon_loss = False
            obs_cur_idx = X3F2AMPCfg.env.num_single_privileged_obs * (X3F2AMPCfg.env.c_frame_stack- 1)
            obs_cur_dim = Single_obs_dim

            lin_vel_loss = True
            lin_vel_idx = X3F2AMPCfg.env.num_single_privileged_obs * (X3F2AMPCfg.env.c_frame_stack- 1) + X3F2AMPCfg.env.lin_vel_idx
            lin_vel_dim = 3

            feet_height_loss = False
            feet_height_idx = X3F2AMPCfg.env.num_single_privileged_obs * (X3F2AMPCfg.env.c_frame_stack- 1) + X3F2AMPCfg.env.feet_height_idx
            feet_height_dim = Feet_height_obs_dim

            height_recon_loss = False
            height_idx = X3F2AMPCfg.env.num_single_privileged_obs * (X3F2AMPCfg.env.c_frame_stack- 1) + X3F2AMPCfg.env.height_idx
    ...
    class runner:
        ...
        save_envs = "x3_f2"
        experiment_name = 'x3_f2'
        run_name = 'AMP_x3_f2_50hz'
        # load and resume
        resume = False
