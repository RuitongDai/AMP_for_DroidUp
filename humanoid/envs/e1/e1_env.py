...
from .e1_cfg_amp import E1AMPCfg
import torch
...
class E1Env(BaseTask):
    def __init__(self, cfg: Union[E1AMPCfg], sim_params, physics_engine, sim_device, headless):
    ...
    def _reset_dofs_amp(self, env_ids, frames):
        ...
        """
        self.dof_pos[env_ids] = E1AMPLoader.get_joint_pose_batch(frames)
        self.dof_vel[env_ids] = E1AMPLoader.get_joint_vel_batch(frames)
        env_ids_int32 = env_ids.to(dtype=torch.int32)
        self.gym.set_dof_state_tensor_indexed(self.sim,
                                              gymtorch.unwrap_tensor(self.dof_state),
                                              gymtorch.unwrap_tensor(env_ids_int32), len(env_ids_int32))
    ...
    def _reset_root_states_amp(self, env_ids, frames):
        ...
        if self.custom_origins:
            ...
            self.root_states[env_ids, :3] += self.env_origins[env_ids]
        # base velocities
        self.root_states[env_ids, 7:10] = E1AMPLoader.get_linear_vel_batch(frames)
    ...
    def visualize_amp_motion(self, _time):
        ...
        self.dof_pos[:, 0:self.num_actions] = visual_motion_frame[7  : 20]
        self.dof_vel[:, 0:self.num_actions] = visual_motion_frame[32 : 45]
    ...
    def _reward_torso_gravity(self):
        torso_quat = self.rigid_body_quat[:, 13]
    ...
    def _reward_torso_ang_vel_xy(self):
        torso_quat = self.rigid_body_quat[:, 13]
        torso_ang_vel = quat_rotate_inverse(torso_quat, self.rigid_body_ang[:, 13])
        rew = torch.sum(torch.square(torso_ang_vel[:, :2]), dim=1)
        return rew

    def _reward_torso_lin_vel_z(self):
        # Penalize z axis base linear velocity
        torso_lin_vel = self.rigid_body_vel[:, 13]
    ...
    def _reward_waist_pos(self):
        return torch.norm(self.dof_pos[:, 12])
