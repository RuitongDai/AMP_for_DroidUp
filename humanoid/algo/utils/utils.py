...
def unpad_depth_trajectories(trajectories, masks):
    return (
        trajectories.transpose(1, 0)[masks.transpose(1, 0)]
        .view(-1, trajectories.shape[0], trajectories.shape[-2], trajectories.shape[-1])
        .transpose(1, 0)
    )
...
def print_dict_aligned(title: str, d: dict, indent: int = 2, width: int = 75):
    """美观打印配置字典，支持长列表换行与对齐"""
    print(f"\n{title}:")
    if not d:
        print(" " * indent + "(empty)")
    ...
    for k, v in d.items():
        key_str = f"{' ' * indent}{str(k):<{max_key_len}} : "
        # 如果值太长或是列表/字典，用 pprint 自动换行缩进
        if isinstance(v, (list, dict, tuple)) and len(str(v)) > width // 2:
            pp_str = pprint.pformat(v, width=width, compact=True, indent=indent + 4)
            print(f"{key_str}\n{' ' * (indent + max_key_len + 3)}{pp_str}")
        else:
            print(f"{key_str}{v}")
